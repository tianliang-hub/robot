...
export function createScheduler({ store, sceneManager, logger, metrics, advisor }) {
  ...
  function updateCustomerMood(customer, nextMood) {
    ...
    if (sceneManager.setCustomerMoodVisual) {
  ...
  function markCustomerDemandDone(tableId, taskType) {
    state.customers
      ...
      .forEach((customer) => {
        ...
        customer.patience = Math.min(
          EMOTION_CONFIG.patienceMax,
          customer.patience + EMOTION_CONFIG.recoverPerService
        );
  ...
  function setWaiterState(next) {
    ...
    if (
      ["toTransfer", "delivering", "toWater", "checkingOut", "cleaning", "returning", "confirming", "reporting"].includes(
        next
      )
    ) {
  ...
  function promoteSootheTask(tableId) {
    const target = state.taskQueue.find(
      (item) => item.type === "情绪安抚" && item.tableId === String(tableId)
    );
    if (!target) return;
    if (target.priority >= EMOTION_CONFIG.sootheEmergencyPriority) return;
  ...
  async function processChefOrderLoop() {
    ...
    while (chefOrderQueue.length > 0) {
      ...
      await sceneManager.moveAnchor("chef", SERVICE_POINTS.transferChef, getMoveDuration(900), {
        faceToMove: true
      });
  ...
  async function handleOrderTask(task) {
    ...
    await sceneManager.moveAnchor(
      "waiter",
      SERVICE_POINTS.tableService[task.tableId],
      getMoveDuration(900),
      { withPath: true, faceToMove: true, stopDistance: ROBOT_SAFE_DISTANCE * 0.2 }
    );
    ...
    await sceneManager.moveAnchor("waiter", SCENE_POINTS.chef, getMoveDuration(850), {
      withPath: true,
      faceToMove: true,
  ...
  async function handleDeliverTask(task) {
    ...
    await sceneManager.moveAnchor("waiter", SERVICE_POINTS.transferPickup, getMoveDuration(1000), {
      withPath: true,
      faceToMove: true,
      stopDistance: 0.15
    });
  ...
  async function handleWaterTask(task) {
    ...
    await sceneManager.moveAnchor("waiter", SCENE_POINTS.waterPoint, getMoveDuration(900), {
      withPath: true,
      faceToMove: true,
  ...
  async function handleCleanupTask(task) {
    ...
    for (const waypoint of waypointList) {
      ...
      await sceneManager.moveAnchor("waiter", waypoint, getMoveDuration(700), {
        withPath: true,
        faceToMove: true,
    ...
    await sceneManager.moveAnchor("waiter", SERVICE_POINTS.recycleService, getMoveDuration(1200), {
      withPath: true,
      faceToMove: true,
      stopDistance: ROBOT_SAFE_DISTANCE * 0.2
    });
  ...
  async function handlePickupTask(task) {
    ...
    await sceneManager.moveAnchor("waiter", SERVICE_POINTS.fridgeService, getMoveDuration(1050), {
      withPath: true,
      faceToMove: true,
      stopDistance: ROBOT_SAFE_DISTANCE * 0.2
    });
  ...
  async function executeTask(task) {
    ...
    await sceneManager.moveAnchor("waiter", SCENE_POINTS.standby, getMoveDuration(1000), {
      withPath: true,
      faceToMove: true,
      stopDistance: 0
    });
  ...
  function handleAction(action, tableId) {
    ...
    if (!isActionAllowed(status, action)) {
      advisor.explainStateConflict(
        `桌台${key}当前为${TABLE_STATE_LABEL[status]}，不能执行动作${action}`
      );
      return false;
    }
    if (action === "order") {
  ...
  function startEmotionEngine() {
    ...
    emotionTimer = setInterval(() => {
      ...
      state.customers.forEach((customer) => {
        const hasPendingDemand = customer.demandQueue.length > 0;
        if (!hasPendingDemand) return;
        const reason = findDemandDecayReason(customer);
        const reasonDecay = reason ? PATIENCE_DECAY_RULES[reason] ?? EMOTION_CONFIG.decayPerSecond : EMOTION_CONFIG.decayPerSecond;
        ...
        if (
          ...
          addTask(
            "情绪安抚",
            customer.tableId,
            EMOTION_CONFIG.sootheEmergencyPriority,
            "emotion",
            true
          );
